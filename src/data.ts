...
export const quizSections: QuizSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      item("4. Output constraints : Output constraints are the last line of defense before Claude's response reaches your parser. You should specify exactly what you need, including field names, types, length limits, whether to include preamble, and what to do when data is absent. Use structured output features in cases when the format must be machine-readable.", "4. Output constraints : 출력 제약 조건은 Claude의 응답이 parser에 도달하기 전 마지막 방어선입니다. 필드 이름, 타입, 길이 제한, preamble 포함 여부, 데이터가 없을 때 무엇을 할지 등 필요한 것을 정확히 지정해야 합니다. 형식이 반드시 기계가 읽을 수 있어야 하는 경우에는 구조화된 출력 기능을 사용하세요.")
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      item("2-1-5. Moving output control from the prompt into the API with structured outputs", "2-1-5. 구조화된 출력으로 출력 제어를 프롬프트에서 API로 옮기기"),
      ...
      item("That works most of the time, but the prompt is a request, so a model can still return a stray sentence, a wrong field name, or malformed JSON that breaks the parser downstream.", "대부분의 경우에는 작동하지만, 프롬프트는 요청일 뿐이므로 모델은 여전히 불필요하게 끼어든 문장, 잘못된 필드 이름, 또는 하위 파서를 깨뜨리는 형식이 잘못된 JSON을 반환할 수 있습니다."),
      ...
      item("It is called structured outputs, and instead of asking for a shape in words, you hand the API a JSON schema, and the model is constrained at generation time to produce output that matches it.", "그것은 구조화된 출력이라고 하며, 말로 형태를 요청하는 대신 API에 JSON 스키마를 넘기면 모델은 생성 시점에 그 스키마와 일치하는 출력을 만들도록 제약됩니다."),
      item("This technique is constrained decoding: as Claude generates each token, the API only allows tokens that keep the output valid against your schema, so a response that violates the schema cannot be produced in the first place.", "이 기법은 제약 디코딩입니다. Claude가 각 토큰을 생성할 때 API는 스키마에 대해 출력이 유효하게 유지되는 토큰만 허용하므로, 스키마를 위반하는 응답은 처음부터 생성될 수 없습니다."),
      item("Structured outputs cover two situations that show up in real pipelines.", "구조화된 출력은 실제 파이프라인에서 나타나는 두 가지 상황을 다룹니다."),
      ...
      item("JSON outputs constrain the final response.", "JSON 출력은 최종 응답을 제약합니다."),
      item("You set the output_config.format parameter with type json_schema and your schema, and Claude returns valid JSON in the response text that matches that schema every time.", "output_config.format 매개변수에 type json_schema와 스키마를 설정하면, Claude는 매번 그 스키마와 일치하는 유효한 JSON을 응답 텍스트로 반환합니다."),
      item("Reach for this when the model itself is producing the structured payload your code consumes, like extracting fields from a support ticket or formatting an API response, because it removes the parse-and-retry code you would otherwise write around every call.", "지원 티켓에서 필드를 추출하거나 API 응답을 형식화하는 것처럼 모델 자체가 코드가 소비할 구조화된 페이로드를 만들 때 이것을 사용하세요. 그러면 각 호출마다 작성해야 했을 파싱 후 재시도 코드를 제거할 수 있습니다."),
      item("Strict tool use constrains the inputs Claude passes to your tools.", "엄격한 도구 사용은 Claude가 도구에 전달하는 입력을 제약합니다."),
      item("You set strict to true on a tool definition, and the arguments Claude sends to that tool are validated against the input schema before your code runs.", "도구 정의에서 strict를 true로 설정하면, Claude가 그 도구에 보내는 인자가 코드가 실행되기 전에 입력 스키마를 기준으로 검증됩니다."),
      item("Reach for this in agentic loops where a malformed tool argument would crash the function or trigger a wrong action; this helps guarantee the call your code receives already conforms to the contract you defined.", "잘못된 도구 인자가 함수를 중단시키거나 잘못된 행동을 유발할 수 있는 에이전트형 루프에서 이것을 사용하세요. 이렇게 하면 코드가 받는 호출이 이미 정의한 계약을 따른다는 것을 보장하는 데 도움이 됩니다."),
      ...
      item("A prompt-level instruction to return only JSON holds on the cases you tried and then slips on an edge case you did not, which is the exact failure the earlier classification example walked through.", "JSON만 반환하라는 프롬프트 수준의 지시는 시도해 본 경우에는 유지되다가, 시도하지 않은 예외 사례에서 어긋날 수 있습니다. 이것이 앞의 분류 예시가 살펴본 정확한 실패입니다."),
      item("A schema constraint does not slip, because the API enforces it on every token rather than trusting the model to remember the instruction.", "스키마 제약은 어긋나지 않습니다. 모델이 지시를 기억하리라 믿는 대신 API가 모든 토큰에서 그것을 강제하기 때문입니다."),
      ...
      item("The first request on a new schema is slower.", "새 스키마의 첫 번째 요청은 더 느립니다."),
      item("The API compiles your schema into a grammar before it can constrain output, and that compilation adds latency on the first call.", "API는 출력을 제약하기 전에 스키마를 문법으로 컴파일하며, 그 컴파일은 첫 호출에 지연 시간을 추가합니다."),
      item("Compiled grammars are cached for 24 hours from last use, so steady traffic on a stable schema pays the cost once, but a workload that changes schemas constantly pays it repeatedly.", "컴파일된 문법은 마지막 사용 시점부터 24시간 동안 캐시되므로 안정적인 스키마에 대한 꾸준한 트래픽은 비용을 한 번만 지불하지만, 스키마가 계속 바뀌는 작업량은 그 비용을 반복해서 지불합니다."),
      item("Your input token count rises.", "입력 토큰 수가 증가합니다."),
      item("When structured outputs are on, the API adds a system prompt describing the expected format, and that injected prompt is billed like any other input token.", "구조화된 출력이 켜져 있으면 API는 기대 형식을 설명하는 시스템 프롬프트를 추가하며, 이 주입된 프롬프트도 다른 입력 토큰처럼 과금됩니다."),
      ...
      item("A guaranteed schema is not a guaranteed success.", "스키마가 보장된다고 해서 성공이 보장되는 것은 아닙니다."),
      item("Two cases still return output that does not match: a refusal, where the model declines for safety reasons and the response carries stop_reason refusal, and a truncation, where the response hits the max_tokens limit and stops mid-structure with stop_reason max_tokens.", "여전히 일치하지 않는 출력을 반환하는 두 경우가 있습니다. 하나는 모델이 안전상의 이유로 거절하고 응답이 stop_reason refusal을 갖는 거절 상황이며, 다른 하나는 응답이 max_tokens 제한에 도달해 구조 중간에서 stop_reason max_tokens로 멈추는 잘림 상황입니다."),
      item("Your code still checks stop_reason rather than assuming every response parses.", "따라서 코드는 모든 응답이 파싱된다고 가정하지 말고 여전히 stop_reason을 확인해야 합니다."),
      item("It does not combine with message prefilling.", "메시지 미리 채우기와는 함께 사용할 수 없습니다."),
      item("JSON outputs and prefilling the assistant message are incompatible, so a pattern that starts the response for Claude and a pattern that constrains the whole response to a schema cannot run on the same request.", "JSON 출력과 어시스턴트 메시지 미리 채우기는 호환되지 않으므로, Claude의 응답을 미리 시작해 주는 패턴과 전체 응답을 스키마로 제약하는 패턴은 같은 요청에서 함께 실행될 수 없습니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("Extended Thinking: Turning reasoning on, calibrating effort, and reading it back correctly", "확장 사고: 추론을 켜고, 노력 수준을 조정하고, 되돌아온 추론을 올바르게 읽기"),
      ...
      item("Turn it on, and the model writes out its step-by-step reasoning first, then gives you the final answer.", "이를 켜면 모델은 먼저 단계별 추론을 작성한 뒤 최종 답변을 제공합니다."),
      item("Your job is to decide when that extra work is worth the cost and to handle the reasoning it sends back.", "당신의 역할은 그 추가 작업이 비용을 들일 가치가 있는지 결정하고, 모델이 돌려보내는 추론을 처리하는 것입니다.")
    ]
  },
  {
    ...
    items: [
      ...
      item("You'll see this reasoning come back as its own thinking block in the API response, positioned just ahead of the block that holds the actual answer.", "이 추론은 API 응답에서 실제 답변 블록 바로 앞에 위치한 별도의 사고 블록으로 돌아옵니다."),
      item("On the newest models, the thinking block's content is omitted by default; you must request a readable summary through the display setting to see it.", "최신 모델에서는 사고 블록의 내용이 기본적으로 생략됩니다. 이를 보려면 표시 설정을 통해 읽을 수 있는 요약을 요청해야 합니다."),
      item("On current models reasoning is adaptive: you enable it with the thinking parameter where it is not already on by default, and the model decides how much reasoning each request needs.", "현재 모델에서 추론은 적응형입니다. 기본적으로 켜져 있지 않은 경우 thinking 매개변수로 활성화하고, 모델이 각 요청에 필요한 추론 양을 결정합니다."),
      item("You tune depth with the effort setting rather than a fixed token budget.", "깊이는 고정된 토큰 예산이 아니라 노력 수준 설정으로 조정합니다."),
      item("The older budget_tokens control is deprecated and, on the newest model generations, returns a 400 error.", "이전의 budget_tokens 제어는 더 이상 권장되지 않으며, 최신 모델 세대에서는 400 오류를 반환합니다."),
      item("That reasoning isn't free; thinking tokens cost the same as output tokens, so running a simple task at high effort means paying for accuracy you don't need.", "그 추론은 무료가 아닙니다. 사고 토큰은 출력 토큰과 같은 비용이 들기 때문에, 간단한 작업을 높은 노력 수준으로 실행하면 필요 없는 정확도에 비용을 지불하는 셈입니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("Task shape: Multi-step reasoning where the model has to hold several constraints at once: a math derivation, a multi-hop logic problem, planning a sequence of dependent actions.", "작업 형태: 수학 유도, 다단계 논리 문제, 의존적인 행동 순서 계획처럼 모델이 여러 제약을 동시에 붙잡아야 하는 다단계 추론 작업입니다."),
      item("Extended thinking call: Enable it, with the effort level matched to the depth of the problem.", "확장 사고 호출: 활성화하되, 문제의 깊이에 맞게 노력 수준을 맞춥니다."),
      item("Reason: The reasoning pass is where the model works through dependencies it would otherwise skip.", "이유: 추론 단계는 모델이 그냥 지나칠 수 있는 의존 관계를 처리하는 곳입니다."),
      ...
      item("Task shape: Agentic loops where the model plans across several tool calls.", "작업 형태: 모델이 여러 도구 호출에 걸쳐 계획하는 에이전트형 루프입니다."),
      ...
      item("Reason: Reasoning before a plan reduces wrong-tool selection downstream.", "이유: 계획 전에 추론을 하면 downstream에서 잘못된 도구를 선택할 가능성이 줄어듭니다."),
      item("Note the carry-back rule below, which applies in every tool-use loop.", "아래의 되돌려 전달 규칙에 주의하세요. 이 규칙은 모든 tool-use loop에 적용됩니다.")
    ]
  },
  {
    ...
    items: [
      item("2-2-3. The carry-back rule: thinking blocks must return to the API unchanged", "2-2-3. 되돌려 전달 규칙: 사고 블록은 변경 없이 API로 되돌려 보내야 합니다."),
      item("When extended thinking is on and your conversation uses tools, there's one rule you can't skip: every thinking block you get back has to go back to the API exactly as it arrived on the next turn.", "확장 사고가 켜져 있고 대화가 도구를 사용할 때는 건너뛸 수 없는 규칙이 하나 있습니다. 받은 모든 사고 블록은 다음 턴에 도착한 그대로 API로 되돌려 보내야 합니다."),
      item("Each block comes with a signature that confirms the reasoning wasn't tampered with.", "각 블록에는 추론이 변조되지 않았음을 확인하는 서명이 함께 옵니다."),
      item("If you edit it, summarize it, or drop it, the signature stops matching and the API rejects the request.", "이를 수정하거나 요약하거나 제거하면 서명이 맞지 않게 되고 API가 요청을 거부합니다."),
      item("Redacted thinking blocks work the same way.", "가려진 사고 블록도 같은 방식으로 작동합니다."),
      ...
      item("The most common slip-up is stripping out the thinking block to save context, which ends up breaking your next request.", "가장 흔한 실수는 컨텍스트를 아끼려고 사고 블록을 제거하는 것이며, 결국 다음 요청을 망가뜨립니다."),
      item("If the real worry is how much context piles up from accumulated reasoning, the fix is the context-engineering work we'll cover in this module.", "진짜 걱정이 누적된 추론으로 컨텍스트가 얼마나 쌓이는지라면, 해결책은 이 모듈에서 다룰 컨텍스트 엔지니어링 작업입니다."),
      item("Forward pointer", "다음으로 이어지는 안내"),
      item("This lesson enables reasoning and calibrates its effort setting; it does not cover model selection.", "이 lesson은 추론을 활성화하고 노력 수준 설정을 조정하는 내용을 다루며, 모델 선택은 다루지 않습니다."),
      item("Choosing which model to run, as distinct from whether to enable reasoning, is taught in the MSO Foundations module that precedes this one.", "추론을 켤지 여부와 별개로 어떤 모델을 실행할지 선택하는 내용은 이 모듈 앞의 MSO Foundations 모듈에서 배웁니다."),
      item("Handles well", "잘 처리하는 경우"),
      item("Hard reasoning and planning tasks where a wrong answer is expensive and the extra tokens buy accuracy.", "틀린 답의 비용이 크고 추가 토큰이 정확도를 높여 주는 어려운 추론 및 계획 작업입니다."),
      item("Adds cost or complexity", "비용 또는 복잡성을 추가하는 경우"),
      item("The carry-back requirement in tool-use loops, and an effort setting you now must calibrate.", "도구 사용 루프에서의 되돌려 전달 요구사항과 이제 조정해야 하는 노력 수준 설정입니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("Block type: thinking block\nRole: Assistant (extended thinking only)/Claude\nContains: Claude’s internal reasoning, visible only when extended thinking is enabled\nCritical rule: The block must be passed back to the API unchanged in subsequent turns. The signature verifies the reasoning hasn’t been modified, so any edit or summary breaks the signature and the API rejects the message. Redacted thinking blocks follow the same rule: pass them back as received, even though the content is encrypted and not human-readable.", "Block type: thinking block\nRole: Assistant (extended thinking only)/Claude\nContains: extended thinking이 활성화된 경우에만 보이는 Claude의 내부 추론\nCritical rule: 이 block은 이후 턴에서 변경 없이 API로 다시 전달되어야 합니다. signature는 추론이 수정되지 않았음을 검증하므로, 어떤 편집이나 요약도 signature를 깨뜨리고 API가 메시지를 거부하게 만듭니다. 가려진 thinking block도 같은 규칙을 따릅니다. 내용이 암호화되어 사람이 읽을 수 없더라도 받은 그대로 다시 전달해야 합니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("You should mark parameters as required when Claude requires them to call the tool correctly.", "Claude가 도구를 올바르게 호출하는 데 필요한 매개변수는 필수로 표시해야 합니다."),
      item("You can mark parameters as optional when the tool can operate without them.", "도구가 해당 매개변수 없이도 작동할 수 있다면 선택 사항으로 표시할 수 있습니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("A schema that’s vague, under-described, or missing required fields will produce tool calls that look syntactically correct but pick the wrong tool, pass malformed inputs, or loop unnecessarily.", "모호하거나 설명이 부족하거나 필수 필드가 빠진 스키마는 문법적으로는 맞아 보이지만 잘못된 도구를 선택하거나, 잘못된 입력을 전달하거나, 불필요하게 루프를 도는 도구 호출을 만듭니다."),
      ...
      item("Every additional tool you register increases the surface area Claude has to reason over, so this discipline only pays off when the underlying tools are distinct.", "추가로 등록하는 모든 도구는 Claude가 추론해야 하는 표면적을 늘리므로, 이 규율은 기저 도구들이 서로 구별될 때만 효과가 있습니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("An older SSE-only transport exists but is deprecated, and new integrations should use Streamable HTTP.", "이전 SSE-only transport도 있지만 더 이상 권장되지 않으며, 새로운 통합은 Streamable HTTP를 사용해야 합니다."),
      ...
      item("Write schemas manually when\nNo MCP server covers your use case, or when you need precise control over tool scope and description quality that a general-purpose server does not provide. Before defaulting to manual schemas for scope control, note that the API MCP Connector supports allowlisting and denylisting specific tools per server via MCPToolset configuration. Manual authoring may still be warranted for description quality, but not always for scope.", "Write schemas manually when\n사용 사례를 다루는 MCP 서버가 없거나, 범용 서버가 제공하지 않는 도구 범위와 설명 품질에 대한 정밀한 제어가 필요할 때 직접 작성하세요. 범위 제어 때문에 곧바로 수동 스키마를 선택하기 전에, API MCP Connector가 MCPToolset 구성을 통해 서버별 특정 도구 allowlist와 denylist를 지원한다는 점을 기억하세요. 설명 품질 때문에 수동 작성이 여전히 필요할 수 있지만, 범위 때문에 항상 필요한 것은 아닙니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("Your handler takes each event and applies it to the partial state it's been building up.", "handler는 각 event를 받아 지금까지 쌓아 온 부분 상태에 적용합니다.")
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      item("2-4-3. The rule that keeps your state from getting corrupted: don't act on a partial block", "2-4-3. 상태가 손상되지 않게 하는 규칙: 부분 block에 대해 행동하지 마세요"),
      item("The tool_use block is the one to watch.", "주의해서 봐야 할 것은 tool_use block입니다."),
      item("Its input shows up as a partial JSON string spread across many content_block_delta events, and that string isn't valid JSON until content_block_stop closes the block.", "그 input은 여러 content_block_delta event에 걸쳐 나뉜 부분 JSON 문자열로 나타나며, content_block_stop이 block을 닫기 전까지는 유효한 JSON이 아닙니다."),
      item("If your code tries to parse the input or run the tool before the block closes, it either chokes on malformed JSON or runs with half the arguments missing.", "block이 닫히기 전에 코드가 input을 parse하거나 tool을 실행하려 하면 형식이 잘못된 JSON에서 막히거나 argument가 절반쯤 빠진 상태로 실행됩니다."),
      ...
      item("A turn built from a stream that got cut off partway is incomplete, and the tool_use pairing rules will reject your next request if a half-built tool_use block ends up in the history.", "중간에 끊긴 stream으로 만든 turn은 불완전하며, 반쯤 만들어진 tool_use block이 history에 들어가면 tool_use pairing rule 때문에 다음 요청이 거부됩니다.")
    ]
  },
  {
    ...
    items: [
      ...
      item("The failure that really bites is treating whatever you've collected so far as if it were complete.", "정말 문제가 되는 실패는 지금까지 모은 것을 완성된 것처럼 다루는 것입니다."),
      item("A partial text block shown to a user is just a cosmetic glitch and a partial tool_use block written into history is a structural problem that corrupts the next turn.", "사용자에게 보이는 부분 text block은 단순한 표시상의 문제지만, history에 기록된 부분 tool_use block은 다음 turn을 손상시키는 구조적 문제입니다."),
      ...
      item("Until then, treat what you've accumulated as provisional.", "그때까지는 지금까지 누적된 내용을 확정된 결과가 아니라 임시 내용으로 간주하세요."),
      item("On an interrupted stream, throw away the partial assistant turn instead of saving it to history, then retry the request.", "중단된 stream에서는 부분 assistant turn을 history에 저장하지 말고 버린 뒤 요청을 다시 시도하세요."),
      item("Committing a half-built turn is exactly what breaks the following request.", "반쯤 만들어진 turn을 commit하는 것이 바로 다음 요청을 깨뜨리는 원인입니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("Model selection and keeping multi-turn sessions in budget", "모델 선택과 여러 턴 세션을 예산 안에 유지하기"),
      item("You make one early choice: which model runs the workload.", "초기에 한 가지 선택을 합니다. 어떤 모델이 작업량을 실행할지입니다."),
      item("The Claude family covers a range of cost, latency, and capability tradeoffs, so the model you pick sets the price and speed floor that every later decision moves within.", "Claude 제품군은 비용, 지연 시간, 능력 사이의 여러 절충점을 포괄하므로, 선택한 모델은 이후 모든 결정이 그 안에서 움직이는 가격과 속도의 하한선을 정합니다."),
      item("Once the model is set, the next constraint is the context window: the full span of text the model can take in at once, including your prompt, the conversation so far, and every tool result.", "모델이 정해지면 다음 제약은 context window입니다. 이는 프롬프트, 지금까지의 대화, 모든 tool result를 포함해 모델이 한 번에 받아들일 수 있는 전체 텍스트 범위입니다."),
      ...
      item("In a multi-step agent session running ten or twenty tool calls, the window fills up fast, and once it fills, the agent either compacts (losing detail) or stalls before the task is done.", "열 개나 스무 개의 tool call을 실행하는 다단계 agent session에서는 window가 빠르게 차고, 가득 차면 agent는 압축되어 세부 정보를 잃거나 task가 끝나기 전에 멈춥니다."),
      item("So, the question for any agent workflow is whether you've decided in advance what goes into the context window, what comes back out as a summary, and what never enters at all.", "따라서 모든 agent workflow에서 물어야 할 질문은 context window에 무엇이 들어가고, 무엇이 요약으로 돌아오며, 무엇은 애초에 들어가지 않는지를 미리 결정했는지입니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("The Claude model family currently spans four tiers: Fable, Opus, Sonnet, and Haiku, each optimized for different cost, latency, and capability tradeoffs.", "Claude 모델 제품군은 현재 Fable, Opus, Sonnet, Haiku 네 등급으로 구성되며, 각각 다른 비용, 지연 시간, 능력 간 절충점에 최적화되어 있습니다."),
      item("Sonnet is the balanced default for most production workloads.", "Sonnet은 대부분의 프로덕션 작업량에 대한 균형 잡힌 기본값입니다."),
      item("Haiku is built for speed and cost efficiency on tasks that fit its capability envelope.", "Haiku는 능력 범위에 맞는 작업에서 속도와 비용 효율을 위해 만들어졌습니다."),
      item("Opus handles demanding work above the Sonnet envelope, and Fable is Anthropic's most capable model, built for the most demanding tasks including complex reasoning, advanced coding, research synthesis, and sophisticated agentic workflows where maximum intelligence is the priority.", "Opus는 Sonnet 범위를 넘는 까다로운 작업을 처리하고, Fable은 Anthropic의 가장 뛰어난 모델로 복잡한 추론, 고급 코딩, 연구 종합, 최대 지능이 우선인 정교한 에이전트형 워크플로 같은 가장 까다로운 작업을 위해 만들어졌습니다."),
      item("Confirm the current lineup and model identifiers against platform.claude.com/docs at build time.", "빌드 시점에 platform.claude.com/docs에서 현재 모델 구성과 모델 식별자를 확인하세요."),
      item("The default starting point is Sonnet.", "기본 시작점은 Sonnet입니다."),
      item("Move up to Opus only when an eval set tells you Sonnet isn't meeting your quality bar.", "평가 세트가 Sonnet이 품질 기준을 충족하지 못한다고 알려 줄 때만 Opus로 올리세요."),
      item("Move down to Haiku only when an eval set tells you the quality regression is acceptable at your task, not just to save costs.", "비용 절감만을 위해서가 아니라, 평가 세트가 해당 작업에서 품질 저하가 허용 가능하다고 알려 줄 때만 Haiku로 내리세요."),
      item("Your decision to move models should always be a measured decision.", "모델을 바꾸는 결정은 항상 측정에 기반한 결정이어야 합니다.")
    ]
  },
  {
    ...
    items: [
      ...
      item("Think of the context window as the amount of space Claude can hold in working memory.", "context window를 Claude가 작업 메모리에 담을 수 있는 공간의 양이라고 생각하세요."),
      ...
      item("The cost of not planning for this is a production outage.", "이를 계획하지 않은 대가는 프로덕션 장애입니다.")
    ]
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      item("Prompt caching stores the processing work done on a stable prefix of your request so follow-up requests can reuse it instead of reprocessing the same tokens.", "Prompt caching은 request의 안정적인 prefix에 대해 수행한 processing work를 저장해서 follow-up request가 같은 token을 다시 처리하는 대신 재사용할 수 있게 합니다."),
      ...
      item("For multi-turn sessions with a stable system prompt and tool schemas, caching those prefixes once and reusing them across turns is the highest-leverage cost reduction available.", "안정적인 system prompt와 tool schema가 있는 multi-turn session에서는 그 prefix들을 한 번 cache하고 turn 사이에서 재사용하는 것이 가장 효과가 큰 비용 절감 방법입니다."),
      ...
      item("Use it during development to verify your context budget assumptions hold against real tool outputs, not just test fixtures, and in production to gate requests that would exceed the window before they error.", "개발 중에는 test fixture가 아니라 실제 tool output에 대해 context budget 가정이 맞는지 검증하는 데 사용하고, 프로덕션에서는 window를 초과할 request를 error 전에 gate하는 데 사용하세요.")
    ]
  },
  {
    ...
    items: [
      ...
      item("The search-across-rounds path removes that infrastructure and the staleness that comes with it, since the model reads the current files at query time, at the cost of spending more tokens and time per query and giving you a less inspectable process.", "search-across-rounds path는 모델이 query time에 현재 file을 읽기 때문에 그 infrastructure와 staleness를 제거하지만, query마다 더 많은 token과 time을 쓰고 process가 덜 inspectable해지는 비용이 있습니다."),
      item("For a stable reference corpus queried with simple lookups, the index is worth owning.", "간단한 lookup으로 query되는 안정적인 reference corpus라면 index를 소유할 가치가 있습니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("The most critical mistake in agent development is choosing the wrong pattern at the start.", "agent development에서 가장 치명적인 실수는 시작할 때 잘못된 pattern을 선택하는 것입니다."),
      ...
      item("Choose a workflow when you can enumerate the exact steps in code.", "code로 정확한 step을 열거할 수 있을 때 workflow를 선택하세요."),
      item("Choose an agent when you can specify the goal and the tools but not the exact path.", "goal과 tool은 지정할 수 있지만 정확한 path는 지정할 수 없을 때 agent를 선택하세요."),
      ...
      item("Choose a workflow when observability with standard tooling is required.", "표준 tooling을 통한 observability가 필요할 때 workflow를 선택하세요."),
      ...
      item("Choose an agent when the task requires creative sequencing of available tools.", "task가 사용 가능한 tool을 창의적인 순서로 조합해야 할 때 agent를 선택하세요.")
    ]
  },
  {
    ...
    items: [
      ...
      item("You can write the loop directly against the Messages API, which gives you full control and full responsibility.", "Messages API를 상대로 loop를 직접 작성할 수 있으며, 이는 완전한 제어권과 완전한 책임을 줍니다."),
      ...
      item("Or you can use Claude Managed Agents (currently in public beta), where Anthropic runs the loop and the sandbox and your application streams events in and results back.", "또는 Claude Managed Agents(현재 공개 베타)를 사용할 수 있습니다. 여기서는 Anthropic이 loop와 sandbox를 실행하고 application은 event를 보내고 result를 다시 받습니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("What you own: The full loop, tool execution, context management, retries, and exit conditions.", "What you own: 전체 loop, tool execution, context management, retry, exit condition을 직접 소유합니다."),
      item("Nothing is provided for you.", "자동으로 제공되는 것은 없습니다."),
      item("Choose this when: You need full control over each step, you have constraints a library does not accommodate, or you are teaching yourself how the loop works before adding abstraction.", "Choose this when: 각 step에 대한 완전한 제어권이 필요하거나, library가 수용하지 못하는 constraint가 있거나, abstraction을 추가하기 전에 loop가 어떻게 작동하는지 직접 배우고 싶을 때 선택하세요."),
      ...
      item("Confirm current default behavior against the Agent SDK reference at build time.", "빌드 시점에 Agent SDK reference를 기준으로 현재 기본 동작을 확인하세요."),
      ...
      item("Currently in public beta, all endpoints require the managed-agents-2026-04-01 beta header and behaviors may be refined between releases.", "현재 공개 베타이며, 모든 endpoint에는 managed-agents-2026-04-01 beta header가 필요하고 release 사이에 behavior가 조정될 수 있습니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("A system prompt that names the specific task and the tools available for it produces more consistent behavior.", "구체적인 task와 그 task에 사용할 수 있는 tool을 명시하는 system prompt는 더 일관된 behavior를 만듭니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("Start with the minimum set required for the task and add tools only when a specific gap in capability is confirmed.", "task에 필요한 최소 set으로 시작하고, capability의 구체적인 gap이 확인될 때만 tool을 추가하세요."),
      item("When agents are the right call: Goal-directed tasks where the exact path cannot be enumerated in advance. Handling variable inputs that would require dozens of conditional branches in a workflow.", "When agents are the right call: 정확한 path를 미리 열거할 수 없는 goal-directed task에 적합합니다. workflow에서 수십 개의 conditional branch가 필요할 variable input을 처리할 때도 적합합니다."),
      item("What you take on when you use an Agent: Agents add behavioral complexity: the path through the task emerges from the model's reasoning over accumulated context rather than from explicit branching logic in your code.", "What you take on when you use an Agent: agent는 behavioral complexity를 추가합니다. task를 통과하는 path는 code의 explicit branching logic이 아니라 누적된 context에 대한 model의 reasoning에서 나옵니다."),
      item("Observability requires transcript-level tooling rather than standard operational logging.", "observability에는 표준 operational logging이 아니라 transcript-level tooling이 필요합니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("Get the governing constraint named at the start, because the wrong client configuration is much more expensive to undo after the agent is wired than to set correctly the first time.", "처음에 governing constraint의 이름을 명확히 하세요. agent가 이미 연결된 뒤 잘못된 client configuration을 되돌리는 것은 처음부터 올바르게 설정하는 것보다 훨씬 비쌉니다."),
  ...
  },
  {
    ...
    items: [
      ...
      item("Too little state in-persistent storage strips the agent of memory across sessions, because anything not written down disappears the moment the conversation ends.", "영구 저장소에 state가 너무 적으면 적어 두지 않은 것은 conversation이 끝나는 순간 사라지므로 agent가 session 간 memory를 잃습니다."),
      ...
      item("Scope: Summarized memory\nWhat persists: A condensed version of prior conversation is generated and injected at the start of the next session.\nCost: Lower token cost per session than replaying full history, but the summarization step drops detail that was in the original.\nWhen to use: Long-running conversational agents where the full history would outgrow the context budget before the conversation is done.\nWhat you lose: Any detail the summarizer did not preserve. The agent only sees what the summarization prompt chose to keep.", "Scope: Summarized memory\nWhat persists: 이전 conversation의 압축된 version을 생성해 다음 session 시작 시 주입합니다.\nCost: 전체 history를 replay하는 것보다 session당 token cost는 낮지만 summarization step에서 원본 detail이 빠집니다.\nWhen to use: conversation이 끝나기 전에 전체 history가 context budget을 넘을 long-running conversational agent에 사용합니다.\nWhat you lose: summarizer가 보존하지 않은 모든 detail을 잃습니다. agent는 summarization prompt가 남기기로 한 것만 봅니다."),
  ...
  },
  {
    ...
    items: [
      item("2-7-4. Current availability: Skills on the Messages API", "2-7-4. 현재 사용 가능 여부: Messages API에서의 Skills"),
      ...
      item("When you delegate a task to a subagent, it starts with a clean context.", "subagent에 task를 delegate하면 깨끗한 context에서 시작합니다."),
      ...
      item("This matters at agent design time: if you are wiring a subagent to perform a task that depends on specific instructions, those instructions need to be registered against the subagent, not assumed to carry over from the parent.", "이는 agent design time에 중요합니다. 구체적인 instruction에 의존하는 task를 수행하도록 subagent를 wiring한다면, 그 instruction은 parent에서 carry over된다고 가정하지 말고 subagent에 등록해야 합니다.")
    ]
  },
  {
    ...
    items: [
      ...
      item("Images, PDFs, and high-volume processing", "이미지, PDF, 대량 처리"),
      item("Up to now you've been managing what Claude remembers between turns.", "지금까지는 Claude가 턴 사이에서 무엇을 기억하는지를 관리했습니다."),
      item("Multimodal ingestion shifts the question to what you're sending in: every image and PDF consumes context budget before Claude reads a single character of your prompt, which changes how you structure requests and what you can fit in one.", "멀티모달 입력은 질문을 무엇을 보내는가로 옮깁니다. 모든 이미지와 PDF는 Claude가 프롬프트의 한 글자를 읽기 전에 컨텍스트 예산을 소비하며, 이는 요청 구조와 한 번에 넣을 수 있는 양을 바꿉니다."),
      item("When you have thousands of inputs to process, sending one request at a time and waiting for each response stops making sense.", "처리할 입력이 수천 개라면 요청을 하나씩 보내고 각 응답을 기다리는 방식은 더 이상 적절하지 않습니다."),
      item("The Batch API is how you handle that volume without blocking your application.", "Batch API는 애플리케이션을 막아 두지 않고 그 규모를 처리하는 방법입니다.")
    ]
  },
  {
    ...
    items: [
      item("2-8-1. Image token cost: Calculate before you commit", "2-8-1. 이미지 토큰 비용: 결정하기 전에 계산하세요"),
      item("Images are not free in terms of context budget.", "이미지는 컨텍스트 예산 측면에서 무료가 아닙니다."),
      item("Claude views images in patches: each 28×28-pixel block of the image is one visual token, so an image costs ⌈width / 28⌉ × ⌈height / 28⌉ visual tokens.", "Claude는 이미지를 패치 단위로 봅니다. 이미지의 각 28×28픽셀 블록이 시각 토큰 하나이므로 이미지 비용은 ⌈width / 28⌉ × ⌈height / 28⌉ 시각 토큰입니다."),
      item("A 1,000 × 1,000 pixel image is ⌈1000/28⌉ × ⌈1000/28⌉ = 36 × 36 patches, about 1,296 visual tokens.", "1,000 × 1,000픽셀 이미지는 ⌈1000/28⌉ × ⌈1000/28⌉ = 36 × 36 패치, 약 1,296개의 시각 토큰입니다."),
      item("At that rate, ten high-resolution screenshots consume as much context as a detailed system prompt.", "그 정도라면 고해상도 스크린샷 10장은 자세한 시스템 프롬프트만큼의 컨텍스트를 소비합니다."),
      item("Each model also has a maximum native image resolution, expressed as a long-edge limit and a visual-token limit, and these limits differ by model tier.", "각 모델에는 긴 변 한도와 시각 토큰 한도로 표현되는 최대 기본 이미지 해상도가 있으며, 이 한도는 모델 등급마다 다릅니다."),
      item("The newest models accept substantially larger images than the standard tier.", "최신 모델은 표준 등급보다 훨씬 큰 이미지를 받습니다."),
      item("Images larger than either limit are downscaled before processing, so the formula runs on the scaled dimensions.", "둘 중 하나의 한도보다 큰 이미지는 처리 전에 축소되므로 공식은 축소된 치수에 적용됩니다."),
      item("Confirm the current per-tier limits against the Vision page (Resolution and token cost) at build time; the limits have changed between model generations and will again.", "빌드 시점에 Vision 페이지의 Resolution and token cost를 기준으로 현재 등급별 한도를 확인하세요. 한도는 모델 세대 사이에 바뀌었고 앞으로도 바뀔 수 있습니다."),
      item("The calculation matters at design time.", "이 계산은 설계 시점에 중요합니다."),
      item("If you are building a pipeline that processes images, measure the token cost of a typical production image against your model's context limit before you write the ingestion code.", "이미지를 처리하는 파이프라인을 만든다면 입력 코드를 작성하기 전에 일반적인 프로덕션 이미지의 토큰 비용을 모델 컨텍스트 한도와 비교해 측정하세요."),
      item("The fix for an over-budget pipeline is often a ten-minute image resize step.", "예산을 초과하는 파이프라인의 해결책은 종종 10분짜리 이미지 크기 조정 단계입니다."),
      item("If you discover this after deployment, it takes even longer.", "배포 후에 발견하면 더 오래 걸립니다.")
    ]
  },
  {
    ...
    items: [
      item("2-8-2. Different ways to send an image: When each is right", "2-8-2. 이미지를 보내는 여러 방식: 각각이 맞는 상황"),
      item("Inline base64", "Inline base64"),
      item("How it works: Encode the image bytes as a base64 string and include the data directly in the message block.", "작동 방식: 이미지 바이트를 base64 문자열로 인코딩하고 데이터를 메시지 블록에 직접 포함합니다."),
      item("Overhead: The full encoded payload travels with every request, which inflates request size and counts against latency on large images.", "오버헤드: 인코딩된 전체 페이로드가 모든 요청과 함께 이동하므로 요청 크기가 커지고 큰 이미지에서는 지연 시간에 영향을 줍니다."),
      item("When to use: Best for one-off images where adding an upload step would add complexity without a payoff.", "사용할 때: 업로드 단계를 추가해도 이득 없이 복잡성만 늘어나는 일회성 이미지에 가장 적합합니다."),
      item("The same image sent repeatedly multiplies the cost, so reach for a different method if reuse is likely.", "같은 이미지를 반복해서 보내면 비용이 배가되므로 재사용 가능성이 있다면 다른 방법을 선택하세요."),
      item("URL reference", "URL reference"),
      item("How it works: Pass a publicly reachable URL in the source block, and Claude fetches the image at request time.", "작동 방식: 소스 블록에 공개적으로 접근 가능한 URL을 전달하면 Claude가 요청 시점에 이미지를 가져옵니다."),
      item("Overhead: No payload travels with the request, but you take on the dependency that the URL must be stable, public, and reachable at the moment Claude tries to fetch it.", "오버헤드: 페이로드는 요청과 함께 이동하지 않지만, Claude가 가져오려는 순간 URL이 안정적이고 공개되어 있으며 접근 가능해야 한다는 의존성을 떠안게 됩니다."),
      item("When to use: Best when the image is already hosted at a stable public URL you control.", "사용할 때: 이미지가 이미 직접 제어하는 안정적인 공개 URL에 호스팅되어 있을 때 가장 적합합니다."),
      item("Skip it for anything behind auth, anything signed with a short expiry, or anything you can't guarantee will be reachable when the request runs.", "인증 뒤에 있거나 짧은 만료 시간으로 서명되었거나 요청 실행 시 접근 가능하다고 보장할 수 없는 경우에는 피하세요."),
      item("Files API", "Files API"),
      item("How it works: Upload the file once through a separate API call, receive a file_id, and reference that ID in any future message.", "작동 방식: 별도의 API 호출로 파일을 한 번 업로드하고 file_id를 받은 뒤 이후 메시지에서 그 ID를 참조합니다."),
      item("Overhead: The upload is a one-time cost; every later request carries the ID instead of the bytes, so payload overhead drops to near-zero from that point on.", "오버헤드: 업로드는 일회성 비용입니다. 이후 모든 요청은 바이트 대신 ID를 담으므로 그 시점부터 페이로드 오버헤드가 거의 0으로 떨어집니다."),
      item("Currently in beta and not available on Bedrock or Vertex AI; verify availability for your deployment platform.", "현재 베타이며 Bedrock 또는 Vertex AI에서는 사용할 수 없습니다. 배포 플랫폼에서 사용 가능 여부를 확인하세요."),
      item("When to use: Best when the same image or PDF appears across multiple requests, or when the asset is large enough that re-sending it would dominate request size.", "사용할 때: 같은 이미지나 PDF가 여러 요청에 나타나거나, 자산이 충분히 커서 다시 보내는 것이 요청 크기의 대부분을 차지할 때 가장 적합합니다."),
      item("Also, the cleanest choice when you want asset management to live separately from inference calls, and the right choice for images that appear across multiple conversation turns, since the file_id carries no payload weight as history grows.", "또한 자산 관리를 추론 호출과 분리하고 싶을 때 가장 깔끔한 선택이며, file_id는 기록이 커져도 페이로드 부담을 갖지 않으므로 여러 대화 턴에 걸쳐 등장하는 이미지에 적합합니다.")
    ]
  },
  {
    ...
    items: [
      item("2-8-3. Sending PDFs: The document block", "2-8-3. PDF 보내기: 문서 블록"),
      item("For PDFs, the block type is document rather than image.", "PDF의 경우 블록 유형은 image가 아니라 document입니다."),
      item("The source structure follows the same pattern as images, which means it can be base64, a URL, or a Files API file_id.", "소스 구조는 이미지와 같은 패턴을 따르므로 base64, URL, 또는 Files API file_id가 될 수 있습니다."),
      item("There is no required name field on a document block.", "문서 블록에는 필수 name 필드가 없습니다."),
      item("The block accepts an optional title field for a readable document name, and an optional context field for additional metadata, but neither is required to send a PDF.", "블록은 읽기 쉬운 문서 이름을 위한 선택적 title 필드와 추가 메타데이터를 위한 선택적 context 필드를 받지만, PDF를 보내는 데 둘 다 필수는 아닙니다."),
      item("All other mechanics, including token cost considerations and Files API reuse, apply in the same way.", "토큰 비용 고려와 Files API 재사용을 포함한 다른 모든 작동 방식은 같은 방식으로 적용됩니다."),
      item("{\n  \"type\": \"document\",\n  \"source\": {\n    \"type\": \"base64\",\n    \"media_type\": \"application/pdf\",\n    \"data\": \"<base64-encoded-pdf-bytes>\"\n  },\n  \"title\": \"contract_review.pdf\"\n}", "{\n  \"type\": \"document\",\n  \"source\": {\n    \"type\": \"base64\",\n    \"media_type\": \"application/pdf\",\n    \"data\": \"<base64-encoded-pdf-bytes>\"\n  },\n  \"title\": \"contract_review.pdf\"\n}")
    ]
  },
  {
    ...
    items: [
      item("2-8-4. Applying prompting techniques to multimodal inputs", "2-8-4. 멀티모달 입력에 프롬프팅 기법 적용하기"),
      item("The same prompting techniques from the first section apply to image and PDF analysis.", "첫 섹션의 같은 프롬프팅 기법이 이미지와 PDF 분석에도 적용됩니다."),
      item("A bare \"describe this image\" prompt produces shallow output for the same reason a bare text prompt does as Claude has no target structure to aim for.", "단순한 \"describe this image\" 프롬프트는 단순한 텍스트 프롬프트와 같은 이유로 얕은 출력을 만듭니다. Claude가 목표로 삼을 구조가 없기 때문입니다."),
      item("The difference is that images carry ambiguity that text cannot, which includes overlapping objects, depth and spatial relationships, and partial occlusion.", "차이는 이미지가 텍스트에는 없는 모호성을 가진다는 점입니다. 여기에는 겹치는 객체, 깊이와 공간 관계, 부분 가림이 포함됩니다."),
      item("A prompt for visual analysis should name how Claude should handle each type of ambiguity.", "시각 분석 프롬프트는 Claude가 각 모호성 유형을 어떻게 처리해야 하는지 명시해야 합니다."),
      item("\"If objects overlap, describe each separately and note the overlap\" is a concrete constraint that a text-only prompt would never need.", "\"객체가 겹치면 각각을 따로 설명하고 겹침을 언급하라\"는 텍스트 전용 프롬프트라면 필요하지 않을 구체적인 제약 조건입니다.")
    ]
  },
  {
    ...
    items: [
      item("2-8-5. The Message Batches API: High-volume asynchronous processing", "2-8-5. Message Batches API: 대량 비동기 처리"),
      item("When you need to run the same prompt pattern against hundreds or thousands of inputs, the synchronous API is the wrong model.", "수백 또는 수천 개의 입력에 같은 프롬프트 패턴을 실행해야 한다면 동기 API는 잘못된 모델입니다."),
      item("Each synchronous call blocks until complete.", "각 동기 호출은 완료될 때까지 차단됩니다."),
      item("At scale, that means your application is either burning threads or running thousands of concurrent connections against rate limits.", "규모가 커지면 애플리케이션이 스레드를 소모하거나 속도 제한을 상대로 수천 개의 동시 연결을 실행하게 됩니다."),
      item("The Message Batches API accepts up to 100,000 or 256 MB requests (whichever comes first) in a single batch call.", "Message Batches API는 단일 배치 호출에서 최대 100,000개 또는 256 MB 요청 중 먼저 도달하는 한도까지 받습니다."),
      item("You submit the batch, receive a batch_id, and poll for completion.", "배치를 제출하고 batch_id를 받은 뒤 완료 여부를 주기적으로 확인합니다."),
      item("When the batch finishes, you download the results.", "배치가 끝나면 결과를 다운로드합니다."),
      item("The per-token cost for batch requests is lower than for synchronous ones.", "배치 요청의 토큰당 비용은 동기 요청보다 낮습니다."),
      item("The tradeoff is latency: batch processing is non-deterministic and can take up to 24 hours, often much faster.", "절충점은 지연 시간입니다. 배치 처리는 비결정적이며 최대 24시간이 걸릴 수 있지만, 보통은 훨씬 빠릅니다."),
      item("The pattern suits offline pipelines, evaluation runs, and data processing jobs, not real-time user interactions.", "이 패턴은 실시간 사용자 상호작용이 아니라 오프라인 파이프라인, 평가 실행, 데이터 처리 작업에 적합합니다."),
      item("Use case: A user uploads a photo and expects an immediate classification.\nRight API pattern: Synchronous API.\nWhy: Real-time response is required. Batch latency is unacceptable for interactive use.", "사용 사례: 사용자가 사진을 업로드하고 즉각적인 분류를 기대합니다.\n올바른 API 패턴: 동기 API.\n이유: 실시간 응답이 필요합니다. 대화형 사용에서는 배치 지연 시간을 허용할 수 없습니다."),
      item("Use case: A nightly pipeline classifies 5,000 customer records.\nRight API pattern: Message Batches API.\nWhy: Latency is not a constraint. Batch cost reduction and asynchronous processing are both valuable.", "사용 사례: 야간 파이프라인이 5,000개의 고객 레코드를 분류합니다.\n올바른 API 패턴: Message Batches API.\n이유: 지연 시간이 제약 조건이 아닙니다. 배치 비용 절감과 비동기 처리가 모두 가치 있습니다."),
      item("Use case: An evaluation run tests a new prompt against 2,000 examples.\nRight API pattern: Message Batches API.\nWhy: Offline task with no real-time requirement. Batch is the correct pattern.", "사용 사례: 평가 실행이 2,000개 예시에 대해 새 프롬프트를 테스트합니다.\n올바른 API 패턴: Message Batches API.\n이유: 실시간 요구사항이 없는 오프라인 작업입니다. 배치가 올바른 패턴입니다."),
      item("Use case: A chatbot generates a reply to a user's message.\nRight API pattern: Synchronous API.\nWhy: The user is waiting; batch would introduce unacceptable delay.", "사용 사례: 챗봇이 사용자 메시지에 대한 답변을 생성합니다.\n올바른 API 패턴: 동기 API.\n이유: 사용자가 기다리고 있으므로 배치는 허용할 수 없는 지연을 만듭니다.")
    ]
  },
  {
    ...
    items: [
      item("2-8-6. When multimodal and batch fit together, and when they don't", "2-8-6. 멀티모달과 배치가 잘 맞는 때와 그렇지 않은 때"),
      item("The combination works for offline workloads that reuse the same assets and need structured output across thousands of inputs.", "이 조합은 같은 자산을 재사용하고 수천 개 입력에 걸쳐 구조화된 출력이 필요한 오프라인 작업량에 잘 맞습니다."),
      item("A nightly pipeline classifying images against a fixed taxonomy is the textbook case: Files API removes redundant uploads, Batches API absorbs the latency, structured-output techniques keep results machine-readable.", "고정된 분류 체계에 따라 이미지를 분류하는 야간 파이프라인이 대표적인 사례입니다. Files API는 중복 업로드를 제거하고, Batches API는 지연 시간을 흡수하며, 구조화 출력 기법은 결과를 기계가 읽을 수 있게 유지합니다."),
      item("Two failure modes break the fit.", "두 가지 실패 양상이 이 적합성을 깨뜨립니다."),
      item("The first is misreading latency: reaching for batch in any user-facing flow with an image produces a system that passes tests and fails in production, because the user is waiting and the batch isn't.", "첫 번째는 지연 시간을 잘못 읽는 것입니다. 이미지가 포함된 사용자 대상 흐름에서 배치를 사용하면 테스트는 통과하지만 프로덕션에서 실패하는 시스템이 됩니다. 사용자는 기다리고 있지만 배치는 그렇지 않기 때문입니다."),
      item("The second is underestimating context cost: images and PDFs consume budget before Claude processes any text, so pipelines loading multiple large images per request blow past token limits at scale.", "두 번째는 컨텍스트 비용을 과소평가하는 것입니다. 이미지와 PDF는 Claude가 텍스트를 처리하기 전에 예산을 소비하므로 요청마다 여러 큰 이미지를 로드하는 파이프라인은 규모가 커졌을 때 토큰 한도를 넘습니다."),
      item("Measure token cost on production-scale inputs before you build.", "빌드하기 전에 프로덕션 규모 입력에서 토큰 비용을 측정하세요.")
    ]
  }
];
